...
$$
- Stuff that's real gets captured in most or all of the bootstrapped estimates.    
- Stuff that happens "by chance" is idiosyncratic to only a few estimates, and gets "washed out" in the averaging.\footnote{\tiny The math is a bit more complicated than on the previous slide, because the bootstrapped estimates are correlated with each other.  But they're not perfectly correlated, which is the source of variance reduction.}    
...
- we can often improve a high-variance nonparametric regression model by bagging it.  This is the simplest form of "ensembling" or "model averaging."   
...
- random forests might be the most popular "off the shelf" nonparametric regression technique.  They're effective, fast, and require little or no tuning.  


## "Out-of-bag" error estimation 

Random forests has a nice built-in way to estimate the generalization error of the model.  

- In each bootstrap sample ("bag"), some of your original observations of "in the bag" (on average, about 2/3).  
...
Let's go back to `tree_examples.R` to see random forests in action.  
